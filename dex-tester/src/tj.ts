...
const TraderJoeV2LBPairAbi = [
  ...
  { inputs: [], name: "FeeHelper__FeeTooLarge", type: "error" },
...
interface TraderJoeV2GetQueryBaseDivQuotedRatioContract extends Contract {
    getActiveId: () => Promise<BigNumberish>;
    getBinStep: () => Promise<BigNumberish>;
    getTokenX: () => Promise<string>;
    getTokenY: () => Promise<string>;
  }

const poolAddress = "0x9f8973fb86b35c307324ec31fd81cf565e2f4a63";

const contract = new ethers.Contract(
  poolAddress,
  TraderJoeV2LBPairAbi,
  provider
) as TraderJoeV2GetQueryBaseDivQuotedRatioContract;

async function tester() {
  const tokenToValidate = { index: 0, decimals: 18, symbol: "WETH" };
  const functionName = `getToken${tokenToValidate.index === 0 ? "X" : "Y"}`;
  const res = await contract[functionName]();
  console.log(typeof res);
  console.log(res);
  const tokenX = await contract.getTokenX();
  const tokenY = await contract.getTokenY();
  console.log(tokenX.toLowerCase(), tokenY.toLowerCase());
}
