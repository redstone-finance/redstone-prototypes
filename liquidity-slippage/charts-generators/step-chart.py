...
path = "../results/stepSlippageV3.csv"
version = 4

def appendToArray(start, end, step, array):
    for i in range(int(start), int(end), int(step)):
        array.append(int(i))

prices = []
appendToArray(1e4, 1e5, 1e4, prices)
appendToArray(1e5, 1e6, 2.5e4, prices)
appendToArray(1e6, 1e7, 1e5, prices)

# from 1e5 to 1e7 step 1e5
# prices = [i * int(1e5) for i in range(1, int(1e7 // 1e5) + 1)]
...
def generate_plots(df, current_script_directory):
    ...
    for dex in dex_list:
        ...
        for index, row in token_pairs.iterrows():
            ...
            # Slip$25kAtoB
            pair_data = dex_data[
                (dex_data["TokenA"] == token_a) & (dex_data["TokenB"] == token_b)
            ]
            col_names_buy = [f"Slip${int(price/1e3)}kBtoA" for price in prices]
            col_names_sell = [f"Slip${int(price/1e3)}kAtoB" for price in prices]
            buy_slippages = pair_data[col_names_buy].iloc[0]
            sell_slippages = pair_data[col_names_sell].iloc[0]

            (line_buy,) = plt.plot(
                prices, buy_slippages, label=f"Buy - {token_a}/{token_b}"
            )
            (line_sell,) = plt.plot(
                prices, sell_slippages, label=f"Sell - {token_a}/{token_b}"
            )
        ...
        file_path = os.path.join(
            current_script_directory, f"results{version}/slippage_chart_{dex}.png"
        )
